...
이러한 문제는 기술 스택 자체의 문제가 아니라, Raw 적재부터 정제·집계까지가 하나의 파이프라인에 혼재된 구조에서는 정합성, 재처리, 장애 복구를 단계별로 관리하기 어렵다는 데서 발생합니다.
<br>
<br>

본 프로젝트에서는 유저 행동 이벤트를 **서비스 이벤트와 유사한 형태로 시뮬레이션**하고, 실제 운영 환경에서 발생할 수 있는 정합성·재처리·장애 복구 이슈를 구조적으로 다룰 수 있는 파이프라인 설계를 목표로 했습니다.

이를 위해 Raw 데이터 보존을 전제로 한 레이어 분리, 증분 처리와 전체 재처리를 모두 고려한 테이블/레이어 구조, 데이터 상태에 기반한 파이프라인 실행 제어가 필요하다고 판단했습니다.
<br>
<br>
<br>
<br>
...
본 프로젝트는 Kafka, Spark와 같은 일반적인 스트리밍 구성 요소를 사용하지만, 각 컴포넌트의 역할을 Raw 적재, 정제, 집계로 명확히 분리합니다. 그리고 데이터 상태와 레이어 단위를 기준으로 파이프라인을 제어하는 구조를 목표로 설계되었습니다.
<br>
<br>
...
- **Bronze**
  ...
  - 정합성 이슈 발생 시 재처리 및 결과 비교를 위한 기준 원본 데이터 역할
- **Silver**
...
**기대 효과**
- **레이어별 복구 기준이 명확**한 파이프라인 구조
- 장애 대응 시 복구 범위 판단에 대한 운영자 부담 감소
- 정합성 로직 변경 시 변경 영향 범위의 레이어 단위 제한
- 신규 Feature 추가 시 기존 파이프라인 안정성 유지
<br>
<br>
<br>
